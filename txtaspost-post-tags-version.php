...
/* 
 * Plugin Name:   TXTAsPost Post Tags Version
 * Version:       2.0
...
class TXTAsPost {
		...
		function process_content($content,$post_date){
			...
			$post = preg_split( '/(\r\n|\r|\n)/', $content );
			$tags_input = trim($post[1]);
		   	$post_title = trim($post[0]);
			$post_content = trim(implode ( "\n" , array_slice( $post,2 ) ));
		   	if (empty($post_title)||empty($post_content)) { return false; }

			$id=$this->txtaspost_post($post_title,$post_content,$post_date,$tags_input);
	
			return $id;

		}
		function txtaspost_post($post_title,$post_content,$post_date,$tags_input) {
			global $wpdb;
		...
		function txtaspost_menu() {
		   ...
		   ?>
		   <div class="wrap">
		      <h2>TXT As Post - Post Tags Version</h2>
		     ...
		     <p>TXT format: <b>First line must be title, the second line is post tags and the rest should be content!</b>	</p>
